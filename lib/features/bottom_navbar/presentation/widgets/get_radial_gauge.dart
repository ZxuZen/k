...
import 'package:flutter_steps_tracker/features/bottom_navbar/presentation/widgets/AccelerometerSensor.dart';
...
class _GetRadialGaugeState extends State<GetRadialGauge> {
  ...
  void initState() {
    ...
    _accelerometerSensor.startListening((AccelerometerEvent event) {
      int calculatedSteps = calculateSteps(event);
      setState(() {
        _steps = calculatedSteps.toString();
      });
    });
  }
  ...
  @override
  void dispose() {  
    _accelerometerSensor.stopListening();
    super.dispose();
  }
  ...
  int calculateSteps(AccelerometerEvent event) {
    final double y = event.y;
    final double threshold = 2.0;
    if ((y - previousY).abs() > threshold) {
      steps++;
    }
    previousY = y;
    return steps;
